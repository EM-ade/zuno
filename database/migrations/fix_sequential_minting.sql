...
BEGIN
    ...
    FOR i IN 1..array_length(p_nft_ids, 1) LOOP
        -- Find an unminted item for this collection that matches our reservation
        -- Order by item_index to ensure sequential minting
        SELECT * INTO v_item_record
        FROM items 
        WHERE collection_id = v_collection_id 
        AND minted = false 
        AND mint_signature = p_reservation_token  -- Match the reservation token
        ORDER BY item_index ASC NULLS LAST  -- Ensure sequential ordering
        LIMIT 1;

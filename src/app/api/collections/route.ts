...
export async function GET(request: NextRequest) {
  try {
    ...
    // For each collection, get accurate mint counts and phases
    const collectionsWithPhases = await Promise.all(
      paginatedCollections.map(async (collection) => {
        ...
        const { count: actualMintedCount } = await supabaseServer
          .from("items")
        ...
        const mintCount = actualMintedCount || 0;
        const progress =
